...
La migración fue aplicada localmente con `pnpm exec prisma migrate deploy` contra PostgreSQL `clonse` en `localhost:5432`. `prisma migrate status` quedó sin migraciones pendientes. El verificador `migration/scripts/admin-catalog-verify.mjs` confirma, mediante consultas de metadatos de solo lectura, que las cuatro tablas y sus columnas esperadas existen, que están presentes los cuatro índices únicos de correspondencia y que no hay claves foráneas en estos catálogos. También consulta mediante Prisma los conteos iniciales `0`: `user_levels`, `subnetworks`, `language_variables` y `settings`. Este conteo vacío es intencional: no se ejecutaron imports ni backfills.
