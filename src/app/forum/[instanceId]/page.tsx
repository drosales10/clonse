    ...
    return (
      <main className="public-shell">
        <section className="auth-card">
          ...
          <h1>Foro no disponible</h1>
          <p className="lead">No encontramos esta instancia o no está publicada.</p>
          <Link className="button button-primary" href="/forum">Volver a foros</Link>
        </section>
      </main>
    );
  }
  ...
  return (
    <ClientShell current="explore">
      <section className="profile-panel forum-panel" aria-labelledby="forum-title">
        ...
        <h1 id="forum-title">{catalog.instance.name ?? "Foro comunitario"}</h1>
        {catalog.instance.description ? <p className="lead">{toExcerpt(catalog.instance.description)}</p> : null}
        <div className="forum-category-bar">
          <Link className={!categoryId ? "category-chip category-chip-active" : "category-chip"} href={`/forum/${catalog.instance.id}`}>Todos</Link>
          {rootCategories.map((category) => (
            <Link className={categoryId === category.id ? "category-chip category-chip-active" : "category-chip"} href={`/forum/${catalog.instance?.id}?categoryId=${category.id}`} key={category.id}>{category.title}</Link>
          ))}
        </div>
        <div className="forum-subcategory-list" aria-label="Navegar por subcategorías">
          {rootCategories.map((root) => {
            const children = catalog.categories.filter((category) => category.parentId === root.id);
            return children.length > 0 ? (
              <div className="forum-subcategory-group" key={root.id}>
                <span>{root.title}</span>
                {children.map((category) => (
                  <Link className="text-link" href={`/forum/${catalog.instance?.id}/categories/${category.id}`} key={category.id}>{category.title}</Link>
                ))}
              </div>
            ) : null;
          })}
        ...
        {catalog.topics.length > 0 ? (
              ...
              <article className="forum-topic-card" key={topic.id}>
                <div className="forum-topic-heading">
                  <div>
                    ...
                    <h2><Link href={`/forum/${catalog.instance?.id}/topics/${topic.id}?categoryId=${topic.categoryId}`}>{topic.title}</Link></h2>
                  </div>
          ...
          <p className="empty-state">No hay temas públicos en esta instancia.</p>
        )}
