...
export default async function ForumInstancePage({ params, searchParams }: { params: Promise<{ instanceId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="authenticated-shell">
    ...
    <section className="profile-panel forum-panel" aria-labelledby="forum-title">
      ...
      <div className="forum-subcategory-list" aria-label="Navegar por subcategorías">{rootCategories.map((root) => { const children = catalog.categories.filter((category) => category.parentId === root.id); return children.length > 0 ? <div className="forum-subcategory-group" key={root.id}><span>{root.title}</span>{children.map((category) => <Link className="text-link" href={`/forum/${catalog.instance?.id}/categories/${category.id}`} key={category.id}>{category.title}</Link>)}</div> : null; })}</div>
